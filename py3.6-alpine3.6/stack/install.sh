...
#
# Install packages
# xargs apk add < ${BASEDIR}/packages.txt
# The sed command removes comments and empty lines from the packages file.
cat ${BASEDIR}/packages.prod.txt | sed '/^#/ d' | sed '/^$/d' | xargs apk add

# If we're building the dev image install the dev packages
if [ "$TARGET" = "dev" ] ; then cat ${BASEDIR}/packages.dev.txt | sed '/^#/ d' | sed '/^$/d' | xargs apk add ; fi
